...
import sqlalchemy
from sqlalchemy import Column, String, Integer, DateTime, Boolean, ForeignKey, MetaData
...
class IntervalSchedule(Base):
    ...
    def schedule(self):
        ...
        print self.period
        print self.every
        print dict(**{self.period: self.every})
        print datetime.timedelta(**{self.period: self.every})
        print 'Done'
        return schedules.schedule(datetime.timedelta(**{self.period: self.every}))

    @classmethod
    def get_by_id(cls, dbsession, id):
        return dbsession.query(IntervalSchedule).filter_by(id=id).one()
